...
#define Y end()
#define Z size()
S BWAPI;S Filter;S std;N U=Unit;N TP=TilePosition;N P=Position;A&g=BroodwarPtr;U fo;U us;U ex;U pb;UnitType tb;A bo=4;set<TP>ep;set<U>gw;set<P>eb;
...
I(!e->getRace())bo=9;}
A mi=g->getStaticMinerals();A ac=[&](A t){R s->allUnitCount(t);};A cb=[](A t){R g->canMake(t);};A rm=[&](U u){R vector(mi.G,mi.Y)[f%mi.Z]->getInitialPosition();};A bv=[](TP p){R g->isVisible(p) && !g->getUnitsOnTile(p,IsEnemy&&IsBuilding).Z;};A gt=[](U u){R TP{u->getTargetPosition()};};A sm=s->minerals(),sg=s->gas(),gp=ac(142),rg=sg>200?2:3;size_t st=-1;
...
us=u;A vv=set(ep.G,ep.Y);vv.Q(gt(fo));I(vv.Z){TP r;A d=st;
...
I(pb){TP bl;A d=st;A xx=pb X.x;A yy=pb X.y;
F(A x=xx-9;x<xx+9;x++)F(A y=yy-9;y<yy+9;y++){TP tp{x,y};I(g->canBuildHere(tp,tb)&&(tb==149||g->getClosestUnit(P{tp}, IsMineralField)D(P{tp})>200))I(pb D(P{tp})<d){d=pb D(P{tp});bl=tp;}}
